...
for (j in 1:length(years)){
   ...
   rownames(w) <- as.character(round(seq(ylim[1], ylim[2], by = ylim[3]),2))
   ...
   for (i in 1:nrow(s)){
      ...
      if (!is.null(o)){
         ...
         vv <- round(vv, 2)
         print(range(vv, na.rm = TRUE))
         if (length(tt) > 0){
            for (k in 1:length(tt)){
               if (!is.na(aa[k])){
                  ii <- as.character(vv[k])
  ...
  # if (j == 2) mtext(expression(paste("Vertical acceleration(m"^"2", "/s)")), 2, 2.5, cex = 1.5)    
   if (j == 2) mtext(expression(paste("Vertical speed (m/s)")), 2, 2.5, cex = 1.5)
