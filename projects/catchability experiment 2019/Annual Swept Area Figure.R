...
for (i in 1:length(years)){
   ...
   delta <- round(s$swept.area / 50) * 50
   plot(c(1000, 4500), c(0, 30), type = "n", xaxs = "i", xaxt = "n", yaxt = "n", yaxs = "i", xlab = "", ylab = "")
...
axis(1)
mtext("Swept Area (m^2)", 1, 3, cex = 1.5)


windows()
s <- read.scset(year = 2000:2019, valid = 1)
s <- s[substr(s$tow.id, 2, 2) != "C", ]
boxplot(swept.area ~ year, data = s, ylim = c(1200, 4500), yaxs = "i")
mtext("Year", 1, 2.5, cex = 1.5)
mtext("Swept Area (m^2)", 2, 2.5, cex = 1.5)
